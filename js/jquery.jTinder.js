...
;(function ($, window, document, undefined) {
	...
	Plugin.prototype = {
		...
		dislike: function() {
			...
			panes.eq(current_pane).find(".item .fade-color").css({"background": "red"});
		},
		...
		like: function() {
			...
			panes.eq(current_pane).find(".item .fade-color").css({"background": "green"});
		},
			...
			switch (ev.type) {
				...
				case 'touchmove':
					if(touchStart === true) {
						...
						if (posX >= 0) {
							...
							panes.eq(current_pane).find(".item .fade-color").css({"background": "green", "opacity": opa});
							panes.eq(current_pane).find(".item").css({"color": "white"});
							...
							panes.eq(current_pane).find(".item .fade-color").css({"background": "red", "opacity": opa});
							panes.eq(current_pane).find(".item").css({"color": "white"});
						}
					}
					break;
				...
				case 'touchend':
					...
					if (opa >= 1) {
						...
						panes.eq(current_pane).find(".item .fade-color").css({"background": "transparent"});
						$(".matches .item").css({"color": "inherit"});
					}
					break;
			}
		}
